...
# PAGINAS disponíveis no sistema (Apenas navegação limpa, sem carregar artes aqui)
PAGINAS = [
    {"icone": "⛪", "titulo": "Cultos Gerais",        "modulo": "pages.Cultos_Gears"},
...
def exibir():
    ...
    # Grid de botões: 4 colunas (Apenas os botões verdes do menu)
    cols = st.columns(4)
    ...
    # SE ALGUÉM CLICAR EM UM BOTÃO: Abre a página correspondente aqui embaixo
    if st.session_state.pagina_atual:
        try:
            modulo = importlib.import_module(st.session_state.pagina_atual)
            importlib.reload(modulo) 
            modulo.exibir() # Chama a exibição interna da página (que terá a capa e as artes certinhas)
